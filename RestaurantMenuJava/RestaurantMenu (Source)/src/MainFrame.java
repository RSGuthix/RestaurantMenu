...
public class MainFrame extends JFrame {
	...
	private JScrollPane getItemButtons() {
		...
		JButton[] buttonList = new JButton[22];
		...
		for (final MenuItem itemButton: itemButtons) {
			...
			final JButton createButton = new JButton(itemButton.getName() + " " + itemButton.getCost());
			createButton.setToolTipText(itemButton.getName());
			
			createButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					if (itemButton.getName().contains("Pizza")) {
						for (int i = 0; i<buttonList.length; i++) {
							buttonList[i].setEnabled(false);
						}
					buttonList[11].setEnabled(true);
					buttonList[12].setEnabled(true);
					buttonList[13].setEnabled(true);
					}
					
					else if (itemButton.getName().contains("Crust")) {
						for (int i = 0; i<buttonList.length; i++) {
							buttonList[i].setEnabled(false);
						}
						...
						buttonList[9].setEnabled(true);
						buttonList[10].setEnabled(true);
					
						String toppingNumber = (JOptionPane.showInputDialog("Please enter the number of toppings you would like (0-4): "));
						NumOfToppings = Integer.parseInt(toppingNumber);
						if(NumOfToppings == 0) {
							for (int i = 0; i<buttonList.length; i++) {
							...
							buttonList[8].setEnabled(false);
							buttonList[9].setEnabled(false);
							buttonList[10].setEnabled(false);
							buttonList[11].setEnabled(false);
							buttonList[12].setEnabled(false);
							buttonList[13].setEnabled(false);
						}
					}
					else if (itemButton.getName().contains("Sausage")||itemButton.getName().contains("Pepperoni")||itemButton.getName().contains("Mushroom")||
							itemButton.getName().contains("Anchovy")||itemButton.getName().contains("Black Olives")||itemButton.getName().contains("Banana Peppers")||
							itemButton.getName().contains("Green Peppers")||itemButton.getName().contains("Extra Cheese")) {
							toppingCounter++;
							if(toppingCounter == NumOfToppings || toppingCounter>3) {
								for (int i = 0; i<buttonList.length; i++) {
									buttonList[i].setEnabled(true);
								}
								buttonList[3].setEnabled(false);
			...
			buttonList[buttonListIterator]= createButton;
			buttonListIterator++;
